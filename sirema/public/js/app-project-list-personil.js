...
$(function () {
  ...
  var dt_request_table = $('.datatables-request'),
    ...
    progresObj = {
        ...
        'On Going': { title: 'On Going', class: 'bg-label-warning' },
    };
  ...
  if (dt_request_table.length) {
    ...
    var dt_request = dt_request_table.DataTable({
      ajax: `/api/projects/personil/${user_id}`, // JSON file to add data
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          },
          {
              ...
              render: function (data, type, full, meta) {
                  var actionHtml = '';
                  var progresProject = full['progres'];

                  // Check the value of konfirmasi_project and return corresponding HTML
                  if (progresProject === 'On Going') { // Waiting
                      actionHtml = `<div class="d-flex justify-content-center align-items-center">` +
                                   `<a href="/personil/konfirmasi-project/${full.id}" class="text-body"><i class="ti ti-edit ti-sm me-2"></i></a>` +
                                   `</div>`;
                  } else if (progresProject === 'Selesai') { // Assigned
                      actionHtml = `<div class="d-flex justify-content-center align-items-center">` +
                                   `<a href="/personil/detail-project/project/${full.id}" class="text-body"><i class="ti ti-eye ti-sm me-2"></i></a>` +
                                   `</div>`;
                  }
      ...
      initComplete: function () {
        ...
        .every(function () {
            var column = this;
            var select = $(
            '<select id="ProgresRequest" class="form-select text-capitalize"><option value=""> Select Progres </option></select>'
            )
            .appendTo('.progres_request')
            .on('change', function () {
                var val = $.fn.dataTable.util.escapeRegex($(this).val());
                column.search(val ? '^' + val + '$' : '', true, false).draw();
            });

            Object.keys(progresObj).forEach(function (key) {
            select.append(
                '<option value="' +
                progresObj[key].title +
