...
$(function () {
  ...
  if (dt_user_table.length) {
    var dt_user = dt_user_table.DataTable({
      ajax: `/api/admin/data-users`,
      columns: [
        // columns according to JSON
        { data: null, defaultContent: '', responsivePriority: 2 },  // For responsive control
        { data: 'name' },
        { data: 'role' },
        { data: 'action', defaultContent: '', responsivePriority: 1 }  // For actions
      ],
      columnDefs: [
        {
          // For Responsive
        ...
        render: function (data, type, full, meta) {
            ...
            return "<span class='text-truncate d-flex align-items-center'>" + roleBadgeObj[formattedRole] + formattedRole + '</span>';
        }
        },
        {
            // Actions
            ...
            render: function (data, type, full, meta) {
                return (
                    ...
                    `<a href="/admin/edit-role/user/${full.id}" class="text-body"><i class="ti ti-edit ti-sm me-2"></i></a>` +
                    `</div>`
                    );
            }
        }
      ...
      initComplete: function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="UserRole" class="form-select text-capitalize w-100"><option value=""> Select Role </option></select>'
            )
              .appendTo('.role')
              .on('change', function () {
