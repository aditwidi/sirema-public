...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    ...
    function eventClick(info) {
      ...
      if (offcanvasTitle) {
        offcanvasTitle.innerHTML = 'Detail Request';
      }
    ...
    function selectedCalendars() {
        let selected = [];
        let isViewAllChecked = document.getElementById('selectAll').checked;

        if (isViewAllChecked) {
            // Add all filter values to the 'selected' array
            let allFilters = [].slice.call(document.querySelectorAll('.input-filter'));
            allFilters.forEach(item => {
                selected.push(item.getAttribute('data-value'));
            });
        } else {
            // Add only checked filters to the 'selected' array
    ...
    function fetchEvents(info, successCallback, failureCallback) {
        ...
        $.ajax({
            url: `/api/personil-calendar-events/${user_id}`, // Your API endpoint
            type: 'GET',
            data: { filters: selectedFilters }, // Send filters to the server
            success: function (result) {
                // Map the API response to the format expected by FullCalendar
                let selectedEvents = result.map(function (event) {
                    return {
                        id: event.id,
                        title: event.title,
                        start: event.start,
                        end: event.end, // Handle the end date based on your API response
                        allDay: false, // Adjust based on your API response or requirements
                        extendedProps: {
                            // Map additional properties from your API response here
                            status: event.status,
                            type: event.type,
                            nama_pengaju: event.nama_pengaju,
                            asal_pengaju: event.asal_pengaju,
                            nomor_telepon_pengaju: event.nomor_telepon_pengaju,
                            bentuk_request_id: event.bentuk_request_id,
                            required_personil: event.required_personil,
                        }
                    };
                });
    ...
    let calendar = new Calendar(calendarEl, {
      ...
      eventClassNames: function ({ event: calendarEvent }) {
        // Get the category from the event's extended properties
        const eventStatus = calendarEvent.extendedProps.type; // Adjust the property name as needed

        const colorName = calendarsColor[eventStatus];
        // Background Color
      ...
      eventDidMount: function(info) {
        if (info.view.type.includes('list')) {
          let timeElement = info.el.querySelector('.fc-list-event-time');
          if (timeElement) {
            timeElement.remove();
          }
        }
      ...
      eventClick: function (info) {
        ...
        window.location.href = `/personil/kalender/detail-request/request/${eventId}`;
      },
      datesSet: function () {
        modifyToggler();
      },
