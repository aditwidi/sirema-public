...
$(function () {
  ...
  var dt_request_table = $('.datatables-request'),
    ...
    'disetujui': { title: 'Disetujui', class: 'bg-label-success' },
  ...
  if (dt_request_table.length) {
    ...
    var dt_request = dt_request_table.DataTable({
      ajax: `/api/requests/${user_id}`, // JSON file to add data
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            if (!statusObj[$status]) {
                return '<span class="badge bg-secondary">Unknown</span>'; // Fallback for unmapped statuses
              }
        ...
        },
        {
            ...
            render: function (data, type, full, meta) {
              ...
              if ($status === 'disetujui') {
                // If status is 'Disetujui', only show the view (eye) action
                return (
                    `<div class="d-flex justify-content-center align-items-center">` +
                    `<a href="/user/detail-request/request/${full.id}" class="text-body"><i class="ti ti-eye ti-sm me-2"></i></a>` +
                    `</div>`
                    );
              } else if ($status === 'pending') {
                // For all other statuses, show all actions
                return (
                  `<div class="d-flex align-items-center">` +
                  `<a href="/user/detail-request/request/${full.id}" class="text-body"><i class="ti ti-eye ti-sm me-2"></i></a>` +
                  `<a href="/user/edit-request/request/${full.id}" class="text-body"><i class="ti ti-edit ti-sm mx-2"></i></a>` +
                  `<a href="javascript:;" class="text-body" onclick="deleteRequest(${full.id});"><i class="ti ti-trash ti-sm mx-1"></i></a>` +
                  `</div>`
                );
