...
(function () {
  ...
  const donutChartEl = document.querySelector('#donutChart'),
    ...
    if (typeof donutChartEl !== undefined && donutChartEl !== null) {
      // Just assign the new ApexCharts instance to donutChart, don't re-declare it
      donutChart = new ApexCharts(donutChartEl, donutChartConfig);
      donutChart.render();
  }

  function updateDonutChartData(newLabels, newSeries) {
    // Calculate the total number of requests
    let totalValue = newSeries.reduce((acc, val) => acc + val, 0);  // Sum of all series values

    donutChartConfig.labels = newLabels;
    donutChartConfig.series = newSeries;
    // Update the label to 'Total Request'
    donutChartConfig.plotOptions.pie.donut.labels.total.label = 'Total Request';
    // Update the formatter to display the total value
    donutChartConfig.plotOptions.pie.donut.labels.total.formatter = function(w) {
        return totalValue;
    };
...
function fetchAndUpdateChartData() {
    $.ajax({
      url: '/api/admin/dashboard-data',  // Replace with your API endpoint
      type: 'GET',
      success: function (response) {
        // Assuming response contains an object like { approved: 10, pending: 5, rejected: 2 }
        const newLabels = ['Request Disetujui', 'Request Pending', 'Request Ditolak'];
        const newSeries = [response.approved, response.pending, response.rejected];
        updateDonutChartData(newLabels, newSeries);
      },
